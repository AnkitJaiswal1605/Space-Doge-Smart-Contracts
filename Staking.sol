...
contract SpaceDogeStaking is EIP712, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;
    
    IERC20 stard = IERC20(0x5BbD6f382e5c2153860133D1fa331f6c0B9A4191);
    ...
    constructor() EIP712("SpaceDogeStaking", "1.0.0") {}
    ...
    function withdrawStard(uint amount, bytes calldata signature) external {
        require(_signerAddress == recoverAddress(msg.sender, amount, accountNonce(msg.sender), signature), "invalid signature");
        stard.transferFrom(address(this), msg.sender, amount);
        addressToNonce[msg.sender].increment();
    ...
    function _hash(address account, uint amount, uint nonce) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("MyStard(uint256 amount,address account,uint256 nonce)"),
                        amount,
                        account,
                        nonce
                    )
